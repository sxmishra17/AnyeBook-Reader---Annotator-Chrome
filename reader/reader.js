...
// ─── PDF Renderer ───────────────────────────────────────────
// Lazy rendering: only pages in/near the viewport are rendered.
...
async function loadPDF(file) {
  ...
  setLoadingProgress(40, 'Preparing pages...');
  const firstPage = await pdfDoc.getPage(1);
  const defaultViewport = firstPage.getViewport({ scale: pdfScale });

  // Create placeholder wrappers for all pages (fast — no rendering)
  for (let i = 1; i <= totalPages; i++) {
    ...
    // Placeholder canvas (blank, correct dimensions)
  ...
  setLoadingProgress(60, 'Setting up viewer...');

  // Render page 1 immediately
  await renderPDFPage(container.querySelector('.pdf-page-wrapper[data-page="1"]'));

  // Set up IntersectionObserver for lazy rendering of remaining pages
  pdfPageObserver = new IntersectionObserver((entries) => {
    ...
    root: readerArea,
    rootMargin: '400px 0px'  // Pre-render pages within 400px of viewport
  });

  container.querySelectorAll('.pdf-page-wrapper').forEach(wrapper => {
    if (wrapper.dataset.rendered === 'false') {
      pdfPageObserver.observe(wrapper);
    }
  });
...
/**
 * Render a single PDF page on demand (called by IntersectionObserver)
 */
async function renderPDFPage(wrapper) {
  ...
  // Update wrapper to actual page dimensions (may differ from page 1)
  ...
  if (pdfPageObserver) pdfPageObserver.unobserve(wrapper);
}
...
async function loadFoliate(file, format) {
  ...
  try {
    ...
    await import(chrome.runtime.getURL('lib/foliate/view.js'));
