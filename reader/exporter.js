...
const Exporter = (() => {
  ...
  /**
   * Download a Blob via chrome.downloads API (reliable in extension pages).
   * Falls back to anchor-click if chrome.downloads is unavailable.
   */
  ...
  async function exportNotesAsDocx(bookName, notes, includePages = true) {
    ...
    try {
      ...
      chromeDownloadBlob(blob, `${buildNotesFilename(bookName)}.docx`);
  ...
  async function exportNotesAsPdf(bookName, notes, includePages = true) {
    ...
    try {
      ...
      chromeDownloadBlob(pdfBlob, `${buildNotesFilename(bookName)}.pdf`);
  ...
  function exportNotesAsTxt(bookName, notes, includePages = true) {
    ...
    try {
      ...
      chromeDownloadBlob(blob, `${buildNotesFilename(bookName)}.txt`);
